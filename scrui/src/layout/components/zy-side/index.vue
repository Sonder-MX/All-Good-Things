<template>
  <section class="menu-side" :style="themeStyle">
    <ZyHeaderLogo :isSidebarOpen="isSidebarOpen" v-if="showLogo" />
    <a-menu class="side-menu-list" :selectedKeys="selectedKeys" :openKeys="openKeys" mode="inline">
      <template v-for="item in menuListData">
        ...
        <template v-else>
          <RecursiveMenuItem :key="item.path" :menu-info="item" @menu-click="handleMenuItem" />
        </template>
      </template>
    </a-menu>
  </section>
...
watchEffect(() => {
  ...
  const currentMenuParent = findItemWithParent(
