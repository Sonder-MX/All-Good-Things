<template>
  ...
  <a-drawer
    ...
    <div class="theme-set">
      <a-row align="middle">
        <a-col :span="8">
          <a-select v-model:value="currentThemeName" @change="handleChange">
            <a-select-option v-for="(item, index) in themeList" :key="item.name" :value="item.name">
              {{ item.name }}
            </a-select-option>
          </a-select>
        </a-col>
