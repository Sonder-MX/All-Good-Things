  ...
  <div v-show="getShow">
      ...
      <a-form-item name="code" :rules="[{ required: true, message: '请输入验证码!' }]">
        <div style="display: flex">
          ...
          <div class="login-code" @click="getCaptcha">
            <img :src="state.captchaImg" alt="" style="width: 90px; height: 38px" />
          </div>
        </div>
      </a-form-item>
      <a-form-item>
    ...
    <div>
      <a-space class="login-btn-list">
        <a-button @click="setLoginState('register')" class="login-form-button"> 注册 </a-button>
        <a-button @click="setOtherUser" class="login-form-button"> 其他用户 </a-button>
...
const disabled = computed(() => {
  return !(state.formState.email && state.formState.password)
})
